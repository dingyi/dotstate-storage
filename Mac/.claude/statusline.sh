...
# Custom Claude Code statusline - Created: 2026-01-13T18:08:52.771Z
...
# ---- weather and timezone helpers ----
weather_color() { if [ "$use_color" -eq 1 ]; then printf '\033[38;5;216m'; fi; }  # warm peach
aqi_color() { if [ "$use_color" -eq 1 ]; then printf '\033[38;5;156m'; fi; }      # light green
tz_color() { if [ "$use_color" -eq 1 ]; then printf '\033[38;5;251m'; fi; }       # light gray

WEATHER_CACHE="/tmp/cc_weather_cache"
WEATHER_TTL=600

get_weather() {
  if [[ -f "$WEATHER_CACHE" ]] && [[ $(($(date +%s) - $(stat -f %m "$WEATHER_CACHE" 2>/dev/null || echo 0))) -lt $WEATHER_TTL ]]; then
    cat "$WEATHER_CACHE"
    return
  fi
  weather_data=$(curl -sf --max-time 3 "wttr.in/tianjin?format=%t+%C" 2>/dev/null | tr -d '\n' | head -c 30)
...
AQI_CACHE="/tmp/cc_aqi_cache"
AQI_TTL=600

get_aqi() {
  if [[ -f "$AQI_CACHE" ]] && [[ $(($(date +%s) - $(stat -f %m "$AQI_CACHE" 2>/dev/null || echo 0))) -lt $AQI_TTL ]]; then
    cat "$AQI_CACHE"
    return
  fi
  aqi_data=$(curl -sf --max-time 3 "https://api.waqi.info/feed/tianjin/?token=demo" 2>/dev/null | grep -o '"aqi":[0-9]*' | grep -o '[0-9]*')
  if [[ -n "$aqi_data" ]]; then
    echo "$aqi_data" > "$AQI_CACHE"
    echo "$aqi_data"
  fi
}
...
# ---- render statusline ----
# Line 0: Weather and timezones
weather_line=""
if [ -n "$weather_info" ]; then
  weather_line="$(weather_color)${weather_info}$(rst)"
fi
if [ -n "$aqi_info" ]; then
  if [ -n "$weather_line" ]; then
    weather_line="${weather_line}  $(aqi_color)AQI ${aqi_info}$(rst)"
  else
    weather_line="$(aqi_color)AQI ${aqi_info}$(rst)"
  fi
fi

# Add multiple timezone display
tz_shanghai=$(TZ='Asia/Shanghai' date +"%H:%M")
tz_la=$(TZ='America/Los_Angeles' date +"%H:%M")

if [ -n "$weather_line" ]; then
  weather_line="${weather_line}  $(tz_color)|$(rst)  $(tz_color)SHA ${tz_shanghai}$(rst)  $(tz_color)LA ${tz_la}$(rst)"
else
  weather_line="$(tz_color)SHA ${tz_shanghai}$(rst)  $(tz_color)LA ${tz_la}$(rst)"
fi
